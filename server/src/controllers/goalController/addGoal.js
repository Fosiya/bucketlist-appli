...
// add goals
exports.addGoal = function addGoal (req, res) {
    ...
    connection.query(`INSERT INTO goals(listID, title, completed) VALUES (?, ?, ?)`, [id, title, completed], (error, result) => {
        if(error) {
            console.log(error);
        } else {
            res.status(201).send('Goal successfully added');
        }
    });
};